...
/*③ file.h文件的内容如下：*/#include <stdio.h>
#include <stdlib.h>
#include "play.h"
 int createFile(Player pla[ ]) /*建立初始的数据文件*/
{
 FILE *fp; int n;
 if((fp=fopen("d:\\pla.dat", "wb")) == NULL) /*指定好文件名，以写入方式打开*/
 {
...
while(!feof(fp)) /*文件未结束时循环*/
 {
 i++; fread(&pla[i],sizePla,1,fp); /*再读出下一条记录*/
 }
 fclose(fp); /*关闭文件*/
 return i; /*返回记录条数*/
}
...
{
 FILE *fp; if((fp=fopen("d:\\player.dat", "wb")) == NULL) /*以写的方式打开指定文件*/
{
 printf("can not open file !\n"); /*如果打开失败，输出提示信息*/
exit(0); /*然后退出*/
 }
 fwrite(pla,sizePla,n,fp); fclose(fp); /*关闭文件*/}
